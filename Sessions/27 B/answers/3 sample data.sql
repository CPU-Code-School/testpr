...
--e) give some shelves at least one genre and some shelves multiple genres
;
with x as (
...
--f) fill all genre shelves with 5 copies of books that have matching genre
insert ShelfBook(bookid, ShelfId, Quantity)
...
--g) give all stores one shelf without any books, new shelf sequence should be 1 higher than the current highest shelf sequence for the store
insert Shelf(BookstoreId, ShelfSequence)
...
--h) create a new store (suggested name: Early Bird Books), give it a code 100 more than highest current code
insert Bookstore(BookstoreName, BookstoreCode)
select 'Early Bird Books', (select max(b.BookstoreCode) + 100 from Bookstore b)
